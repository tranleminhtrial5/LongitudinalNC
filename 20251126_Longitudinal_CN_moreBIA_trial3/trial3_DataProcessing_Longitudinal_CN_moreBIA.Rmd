...
df <- df %>%
  mutate(
    ...
    # New variables from segmental ECW/TBW
    ECW_TBW_upper = (ecw_tbw_ra + ecw_tbw_la) / 2,
    ECW_TBW_lower = (ecw_tbw_rl + ecw_tbw_ll) / 2,
  ...
  rename(
    icw = icwIntracellularWater,
    ecw = ecwExtracellularWater,
    tbw = tbwTotalBodyWater,
    slm = slmSoftLeanMass,
    ffm = ffmFatFreeMass,
    smm = smmSkeletalMuscleMass,
    SW_tr = sw_tr,
    SL_tr = sl_tr
  )
...
df <- df %>%
  ...
  mutate(
    ...
    tbw_ffm = ifelse(
      is.na(tbw_ffm) & !is.na(tbw) & !is.na(ffm) & ffm > 0,
      tbw / ffm * 100,
      tbw_ffm
    )
  )
...
# ---------- Báo cáo số lần mỗi biến xuất hiện trong outlier_vars (in ra console) ----------

if (nrow(outlier_info) == 0) {
  cat("Không có outlier nào (theo ngưỡng |z| > 3).\n")
} else {
  # Tách các tên biến trong outlier_vars thành vector
  outlier_var_vec <- strsplit(outlier_info$outlier_vars, split = ",\\s*")
  outlier_var_vec <- unlist(outlier_var_vec)
  outlier_var_vec <- outlier_var_vec[outlier_var_vec != ""]  # bỏ chuỗi rỗng nếu có
  
  # Đếm tần suất từng biến
  outlier_counts <- sort(table(outlier_var_vec), decreasing = TRUE)
  
  cat("\nSỐ LẦN MỖI BIẾN BỊ FLAG LÀ OUTLIER (|z| > 3):\n")
