...
df_change_StableCN <- df_long.analysis %>%
  filter(Change_Type == "StableCN") %>%
  select(USUBJID, k_no, elapsed_years, all_of(var_name)) %>%
  arrange(USUBJID, elapsed_years) %>%
  group_by(USUBJID) %>%
  mutate(
...
write.csv(df_change_StableCN, sprintf("ChangeTable_Variable_%s_StableCN_long.csv", var_name), row.names = FALSE)
cat(sprintf("Done! CSVs saved for variable %s\n", var_name))
```
