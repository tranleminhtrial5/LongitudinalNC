...
bioimpedance_vars <- c(
  ...
  "ecw_tcw", "PA50_total", "ECW_ICW_upper", "PA50_total", "ECW_ICW_lower",
...
stopifnot(is.numeric(df_table1$MMSE))
...
vars_table2 <- intersect(c("height", "weight", "bmi", "whr", "pbcm", "pbf", "bmr","PA50_total", "ecw_tcw", "PA50_total", "tbw_ffm"), names(df_baseline))
...
make_compare_table <- function(df, var, covars, group = "Change_Type") {
  ...
  # Lấy dòng p-value so sánh ProgressiveCN - StableCN
  out <- tibble::tibble(
    Variable = var,
    Mean_StableCN = means$Mean[1],
    Mean_ProgressiveCN = means$Mean[2],
    SD_StableCN = means$SD[1],
    SD_ProgressiveCN = means$SD[2],
