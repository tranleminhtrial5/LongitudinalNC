...
# Customized function to create comparison table with Mean (SD) format
make_compare_table <- function(df, var, covars, group = "Change_Type") {
  ...
  # Compute group summaries (N, Mean, SD)
  means <- df %>%
    group_by(.data[[group]]) %>%
    summarise(
  ...
  # Output table row with formatted means
  out <- tibble::tibble(
    Variable = var,
...
# Generate tables for the two variable groups
...
# Convert to flextable with significant p-value bolded
