...
remove_list <- c("E_237", "F_242", "a_531", "E_391", "G_31", "F_385", "a_281", "K_368") #9962 "K_368"
...
vars_table3 <- c('ECW_ICW_upper','ECW_ICW_lower',
...
bia_vars <- c("MMSE","SNSB_attention","SNSB_language", "SNSB_visuospatial", "SNSB_memory", "SNSB_frontal","bmi","whr","pbcm","pbf","bmr","ecw_tbw","tbw_ffm","PA50_total",
              ...
              "PA_upper","PA_lower", "SW_upper", "SW_lower", "SL_upper", "SL_lower",
  "R5_upper", "R5_lower",
  "Xc5_upper", "Xc5_lower", "z1_upper", "z1_lower", "z5_upper", "z5_lower", "SL_UPPERtoLOWER_ratio", "SW_UPPERtoLOWER_ratio")
...
```
```{r}
# Check từng variable về sự thay đổi qua hàng năm

library(dplyr)

var_name <- "Xc_lower"

df_change_CNtransitMCI <- df_long.analysis %>%
  filter(Change_Type == "CNtransitMCI") %>%
  select(USUBJID, k_no, elapsed_years, all_of(var_name)) %>%
  arrange(USUBJID, elapsed_years) %>%
  group_by(USUBJID) %>%
  mutate(
    Difference = !!sym(var_name) - lag(!!sym(var_name)),
    Year_Diff = elapsed_years - lag(elapsed_years)
  ) %>%
  ungroup()

write.csv(df_change_CNtransitMCI, sprintf("ChangeTable_Variable_%s_CNtransitMCI_long.csv", var_name), row.names = FALSE)
cat(sprintf("Done! CSVs saved for variable %s\n", var_name))
